    ...
    connection.query(sql_zero, function(error, results) {
        if (error)
            ...
            if (request.params.test_id == 1) {
                ...
                let ketFactorsName = ['Общительность (Фактор А)', 'Интеллект (Фактор В)', 'Эмоциональная устойчивость (Фактор С)', 'Доминантность (Фактор E)', 'Экспрессивность, беззаботность (Фактор F)', 'Нормативность поведения (Фактор G)', 'Смелость (Фактор H)', 'Чувствительность (Фактор I)', 'Подозрительность (Фактор L)', 'Мечтательность (Фактор М)', 'Дипломатичность (Фактор N)', 'Тревожность (Фактор O)', 'Экспериментирование (Фактор Q1)', 'Независимость (Фактор Q2)', 'Самоконтроль (Фактор Q3)', 'Напряженность (Фактор Q4)'];
                let ketFactorsClar = [
                                        '',
                                        '',
                                        '',
                ...
                for (let i = 0; i < 16; i++) {
                    for (let j = 1; j < 11; j++) {
                        ...
                        else if ((results[i].Value > ketStansTable[i][j-1]) && (results[i].Value <= ketStansTable[i][j])) {
                            stansBar[i] = j;
                        }
                    }
                }
                ...
                for (let i = 0; i < 16; i++) {
                    if (stansBar[i] <= 3 && stansBar[i] >= 1) {
                        processed_data[0].factors[i] = {
                            name: ketFactorsName[i],
                            description: ketMarks[i][0],
                            clarification: ketFactorsClar[i]
                        }
                    }
                    else if (stansBar[i] <= 5 && stansBar[i] >= 4) {
                        processed_data[0].factors[i] = {
                            name: ketFactorsName[i],
                            description: ketMarks[i][1],
                            clarification: ketFactorsClar[i]
                        }
                    }
                    else if (stansBar[i] <= 7 && stansBar[i] >= 6) {
                        processed_data[0].factors[i] = {
                            name: ketFactorsName[i],
                            description: ketMarks[i][2],
                            clarification: ketFactorsClar[i]
                        }
                    }
                    else if (stansBar[i] <= 10 && stansBar[i] >= 8) {
                        processed_data[0].factors[i] = {
                            name: ketFactorsName[i],
                            description: ketMarks[i][3],
                            clarification: ketFactorsClar[i]
                        }
                    }
                }
