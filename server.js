...
app.get("/check-post-exists", function(request, response){
    ...
    if (accessStatus.status) {
        ...
        const sql_zero = `SELECT * FROM Post WHERE VK_ID = ${accessStatus.vk_user_id};`;
    ...
    if (accessStatus.status) {
        ...
        if (request.body.collection != undefined) {
            ...
            if (request.body.collection.error_msg != undefined){
                ...
                post_data = [request.body.collection.error_msg, accessStatus.vk_user_id];
            }
            else if (request.body.collection.length == 0) {
                sql_add = 'INSERT INTO Post(Error, VK_ID) VALUES (?, ?);';
                post_data = ['no post', accessStatus.vk_user_id];
            }
            else {
